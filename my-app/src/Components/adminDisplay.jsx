...
function Display(props){
    ...
    const Delete=async (e)=>{
        const resp=await fetch(`http://13.233.231.169:8081/api/deleteUser/${specUser._id}`,{
            method:'DELETE',
            headers:{
    ...
    const Update=async ()=>{
        const resp=await fetch(`http://13.233.231.169:8081/api/updateUser/${specUser._id}`,{
            method:'POST',
            headers:{
                'Content-Type':'application/json',
                'auth-token':sessionStorage.getItem('auth-token')
