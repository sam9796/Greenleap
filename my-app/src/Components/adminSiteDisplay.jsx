...
function SiteDisplay(props) {
  ...
  const Delete1 = async (e) => {
    const resp = await fetch(
      `http://localhost:8081/api/deleteClient/${specSite.id}`,
      {
        method: "DELETE",
        headers: {
  ...
  const Update1 = async () => {
    const resp = await fetch(
      `http://localhost:8081/api/updateClient/${specSite.id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          name: specSite.siteName,
          location: specSite.location,
          gps: specSite.gps,
          number: specSite.number,
          capacity: specSite.capacity,
        }),
      }
    );
    const resp1 = await resp.json();
