...
const channel = pusher.subscribe("my-channel");
  ...
  setVisible('hidden');
    ...
    const giveDate= async (event)=>{
      ...
      let arr=k2.split('-')
      let resp1=await fetch('http://13.127.159.13/api/load',{
        method:"POST",
        headers:{
          'Content-Type':'application/json',
          'auth-token':sessionStorage.getItem('token')
      ...
      resp1=await fetch('http://13.127.159.13/api/load',{
        method:"POST",
        headers:{
          'Content-Type':'application/json',
          'auth-token':sessionStorage.getItem('token')
    ...
    const resp1=await fetch('http://13.127.159.13/api/getRoboData',{
      method:"POST",
      headers:{
        'Content-Type':'application/json',
        'auth-token':sessionStorage.getItem('token')
    ...
    const handleCommand=async (event,comm)=>{
      ...
      let robots=[]
        ...
        if(robots.length===0){
          ...
          pauseOnHover: true,});
        return;}
      const resp=await fetch(`http://13.127.159.13/api/roboCommand/${beta}`,{
            method:"POST",
            headers:{
              'Content-Type':'application/json'
            },
