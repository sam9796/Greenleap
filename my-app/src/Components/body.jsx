...
function Body() {
  ...
  const toUpdate = async () => {
    const resp = await fetch("http://13.232.106.215/api/getRobots", {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    ...
    function clearData() {
      ...
      let va = document.getElementById("bodyFirst1");
      if (alp) {
        va.removeChild(alp);
      }
    }
  ...
  const giveDate = async (event) => {
    ...
    let arr = k2.split("-");
    let resp1 = await fetch("http://13.232.106.215/api/load", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "auth-token": sessionStorage.getItem("token"),
    ...
    if (resp2 === "0") {
      toast.error("System Data file does not exit", {
        toClose: 3000,
        closeOnClick: true,
        pauseOnHover: true,
      });
    } else {
      let blob = new Blob([resp2], { type: "text/plain" });
    ...
    resp1 = await fetch("http://13.232.106.215/api/load", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "auth-token": sessionStorage.getItem("token"),
    ...
    if (resp2 === "0") {
      toast.error("Clean Data file does not exit", {
        toClose: 3000,
        closeOnClick: true,
        pauseOnHover: true,
      });
  ...
  const showGraph = async (k1) => {
    ...
    const resp3 = await fetch("http://13.232.106.215/api/getGraph", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "auth-token": sessionStorage.getItem("token"),
      },
      body: JSON.stringify({ month: month1, year: year1, roboId: k1 }),
    });

    const resp4 = await resp3.json();
    if (resp4.success) {
      setChartData({
        labels: resp4.arr.map((val, idx) => idx + 1),
        datasets: [
          {
            label: "No of hours worked",
            data: resp4.arr.map((val) => val),
          },
        ],
      });
    } else if (!resp4.success) {
      toast.error("Graph Data Does Not Exist", {
        autoClose: 3000,
        closeOnClick: true,
        pauseOnHover: true,
      });
  ...
  const showDisplay = async (event) => {
    ...
    showGraph(k1);
    const resp1 = await fetch("http://13.232.106.215/api/getRoboData", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "auth-token": sessionStorage.getItem("token"),
    ...
    if (!resp2.success) {
      toast.error("Robot Data Does Not Exist", {
        autoClose: 3000,
        closeOnClick: true,
        pauseOnHover: true,
      });
  ...
  const handleCommand = async (event, comm) => {
    ...
    if (comm) beta = comm;
      ...
      let alp = event.target.parentNode.previousElementSibling.firstChild;
      beta = alp.value;
      alp.value = "";
    }
    ...
    for (let i in robos) {
      if (robos[i].checked)
        robots.push(robos[i].parentNode.nextElementSibling.textContent);
    }
    if (robots.length === 0) {
    ...
    const resp = await fetch(`http://13.232.106.215/api/roboCommand/${beta}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    <div className="body">
      ...
      >
        ...
        <div
          style={{
            gap: "2vw",
            display: `${visible === "hidden" ? "none" : "block"}`,
          }}
        >
          <div className="date-selector">
            <select
              ...
              onChange={() => {
                let robo1 = document.getElementById("graphy");
                showGraph(robo1.textContent);
              }}
            >
      ...
      >
        ...
        >
          ...
          >
            <thead
              style={{ backgroundColor: "#f6f9fc", color: "#a7b3c1" }}
        ...
        <div className="bodySecondRow">
          ...
          <div
            ...
            onClick={(event) => handleCommand(event, "start")}
          >
          ...
          <div
            className="controlButton"
            onClick={(event) => handleCommand(event, "stop")}
          >
          ...
          <div
            className="controlButton"
            onClick={(event) => handleCommand(event, "clean")}
          >
          ...
          <div
            className="controlButton"
            onClick={(event) => handleCommand(event, "rtrn")}
          >
